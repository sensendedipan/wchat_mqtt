...
#define dhcp_timer_init()         timer3Init()

bool	gFlagDHCPState = false;						/*dhcp成功获取IP*/

DHCP_Get DHCP_GET;

uint8_t	*SRC_MAC_ADDR   = EXTERN_DHCP_MAC;			/*本地MAC地址*/
uint8_t	*GET_SN_MASK    = EXTERN_DHCP_SN;			/*从DHCP服务器获取到的子网掩码*/
uint8_t	*GET_GW_IP      = EXTERN_DHCP_GW;			/*从DHCP服务器获取到的网关地址*/
uint8_t	*GET_DNS_IP     = EXTERN_DHCP_DNS;			/*从DHCP服务器获取到的DNS服务器地址*/
uint8_t	*DHCP_HOST_NAME = EXTERN_DHCP_NAME;   		/*主机名*/
uint8_t	*GET_SIP        = EXTERN_DHCP_SIP;			/*从DHCP服务器获取到的本机IP*/
...
static uint8_t		dhcp_state       = STATE_DHCP_READY;	/*DHCP客户端状态*/
static uint8_t		dhcp_retry_count = 0;					/*DHCP重试次数*/
static uint8_t		dhcp_timeout     = 0;					/*DHCP超时标志位*/
static uint32_t 	dhcp_lease_time;						/*租约时间*/
static uint32_t	    next_dhcp_time  = 0;					/*DHCP超时时间*/                  	  
static uint8_t 	    conflict_flag   = 0;

static uint32_t	    DHCP_XID        = DEFAULT_XID;				
static uint8_t		EXTERN_DHCPBUF[1024];
static RIP_MSG		*pRIPMSG = (RIP_MSG*)EXTERN_DHCPBUF;	/*DHCP消息指针*/


static void  send_dhcp_discover(void);						/*向DHCP服务器发送发现报文*/
static void  send_dhcp_request(void);						/*向DHCP服务器发送请求报文*/
static void  send_dhcp_release_decline(char msgtype);		/*向DHCP服务器发送释放报文*/

static void  reset_dhcp_time(void);        				    /*初始化DHCP计时器*/
static void  check_dhcp_timeout(void);     				    /*检查是否超时*/
static uint8_t check_leased_ip(void);						/*检查获取的IP是否冲突*/
static uint8_t parse_dhcp_msg(uint16_t length);				/*从DHCP服务器接收消息并解析*/
...
*/
static uint8_t parse_dhcp_msg(uint16 length)
{
	uint8_t  svr_addr[4];
	uint16_t svr_port;
	uint16_t len;
...
*/
static uint8_t check_dhcp_state(SOCKET s) 
{
	...
	if (getSn_SR(s)!=SOCK_CLOSED) {            /*socket处于打开状态*/
		...
		if ((len = getSn_RX_RSR(s)) > 0) {	/*接收到数据*/
			type = parse_dhcp_msg(len);		/*解析收到的封包类型*/
		}
...
*@return	无
*/
static uint8_t check_leased_ip(void)
{
#ifdef DHCP_DEBUG
	...
	if (sendto(SOCK_DHCP, (const uint8_t*)"CHECK_IP_CONFLICT", 17, GET_SIP, 5000)) { 
#ifdef DHCP_DEBUG
			printf(" Conflict>\r\n");
#endif
...
*@return	无
*/
static void init_dhcp_client(void)
{
	uint8_t ip_0[4] = {0,};
	DHCP_XID = 0x12345678;
...
{ 
    ...
    switch(dhcpret)
    {
        ...
        case DHCP_RET_UPDATE:							 /*成功获取到IP地址*/              
            w5500SetIP();                                /*将获取到的IP地址写入W5500寄存器*/ 
            gFlagDHCPState = true;
            conflict_flag = 0;                           //! 清零 为下次获取ip作准备       
#ifdef DHCP_DEBUG           
            printf("INFO: w5500 get dynamic ip from DHCP server succeed! \r\n");
#endif
            break;
        
        case DHCP_RET_CONFLICT:                          /*IP地址获取冲突*/ 
#ifdef DHCP_DEBUG            
            printf("WARN: w5500 get dynamic ip from DHCP server failed! \r\n");
#endif
            dhcp_state = STATE_DHCP_READY;
#ifdef DHCP_DEBUG
            printf("WARN : w5500 get dynamic ip retrying! \r\n");
#endif
