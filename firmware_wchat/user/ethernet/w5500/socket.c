...
#define W5500_WAIT_TIME 1000
static uint16_t socket_local_port;
...
uint8_t socket(SOCKET s, uint8_t protocol, uint16_t port, uint8_t flag)
{
	...
	)
	{
		...
		if (port != 0) {
			...
			socket_local_port++; // if don't set the source port, set local_port number.
			IINCHIP_WRITE(Sn_PORT0(s), (uint8_t)((socket_local_port & 0xff00) >> 8));
			IINCHIP_WRITE(Sn_PORT1(s), (uint8_t)(socket_local_port & 0x00ff));
		}
		IINCHIP_WRITE(Sn_CR(s), Sn_CR_OPEN); // run sockinit Sn_CR

		/* wait to process the command... */
		while(IINCHIP_READ(Sn_CR(s))) {
			if (timeout ++ > W5500_WAIT_TIME) {
				return 0;
			} 
...
uint16_t send(SOCKET s, const uint8_t *buf, uint16_t len)
{
	...
	// copy data
    if (ret != 0) send_data_processing(s, (uint8_t *)buf, ret);
...
uint16_t sendto(SOCKET s, const uint8_t *buf, uint16_t len, uint8_t *addr, uint16_t port)
{
	uint16_t ret = 0;
	uint32_t timeout = 0;
    const static uint32_t w5500_block_time = 2000000;
